...
begin
    ...
    begin
        ...
        try
            ...
            begin
                GetTextExtentPoint32(dc, PChar(ListBox.Items[i]),
                    Length(ListBox.Items[i]), size);
...
begin
    ...
    if mRFrameList <> nil then Exit;        // 언도킹시 FormShow 재호출 중복 생성 방지
    ...
    TBaseCAN.mRDebug := gSysenv.rDevelop.rCanReadLog;
    TBaseCAN.mRDebug := gSysenv.rDevelop.rCanWriteLog;
...
var
    Frame: PCanFrame;
begin
    if (lbWFrame.Count = 0) or (lbWFrame.ItemIndex < 0) then Exit;
...
var
    Frame: PCanFrame;
begin
    if (lbCyWFrame.Count = 0) or (lbCyWFrame.ItemIndex < 0) then Exit;
...
var
    ...
    Frame: PCanFrame;
begin
    if  (mRFrameList.Count = 0) then Exit;
...
var
    Frame: PCanFrame;
begin
    if (lbRFrame.Count = 0) or (lbRFrame.ItemIndex < 0) then Exit;
