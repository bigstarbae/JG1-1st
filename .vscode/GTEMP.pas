...
uses
    ...
    TypInfo, UDSDef, ClipBrd, LangTran, MsgForm, UDSCarConfig;
...
begin
    ...
    if Assigned(mBuckTester) then
        FreeAndNil(mBuckTester);

    if Assigned(mAPTTester) then
        FreeAndNil(mAPTTester);

    if Assigned(mODSTester) then
        FreeAndNil(mODSTester);

    if Assigned(mDTCReader) then
        FreeAndNil(mDTCReader);

    if Assigned(mECUVer) then
        FreeAndNil(mECUVer);

    if Assigned(mECUVerList) then
        FreeAndNil(mECUVerList);

    if Assigned(mHVTester) then
        FreeAndNil(mHVTester);

    if Assigned(mDIOMemCtrler) then
...
//    mMoveCtrler[tmCushExt] := TDIODirectMoveCtrler.Create(mExtCon, pspCushExt);
    ...
    mMtrTilt.OnStopCond := nil;

    mMtrSlide.SetMoveParam(7, 2);

    //mMtrSlide.OnStopCond := SlideMtrStopCond; // 근접 센서로 정지 조건
...
begin
    ...
    mDIO.SetIO(DO_RJ1_HI_CAR, (mCurModelType.IsRJ1 and mCurModelType.IsDrvPos) or mCurModelType.IsHI);
    mDIo.SetIO(DO_RJ1_APSM_PASS, mCurModelType.IsRJ1 and mCurModelType.IsPsgPos);
    mDIO.SetIO(DO_JG1_CAR, mCurModelType.IsJG1);
    mDIo.SetIO(DO_CAN_100K, mCurModelType.IsHI);
...
begin
    ...
    case mFuncState of
        0:
            begin
                mMtrOper.Init([mMtrHeight], [twForw]);
                Inc(mFuncState);
...
begin
    ...
    case TestStatus of
        ...
        msLimitReaded: // IMS 리미트 읽음
            begin
                if Motor.IMSCtrler.GetLimit(Motor.ID) then
                begin

                    for MtrIt := Low(TMotorOrd) to MtrOrdHi do
                    begin
                        if mMotors[MtrIt].Limit then
                        begin
                            mDBX.SetData(roRsLimit, mMotors[MtrIt].Limit);
                        end;
                    end;
...
//                    Exit;
                ...
                if not (mElecState = _WS_ELEC_END) then
                    Exit;
...
{$ENDIF}
        ...
        _WS_READ_ECU_VER:
            begin
                ...
                case mECUVer.FSMReadWithRetry(1) of
                    -1:
                        begin
                    ...
                    1:
                        begin
                            ...
                            mDBX.SetData(roDataEcuPartNo, mECUVer.PartNo);
                            mDBX.SetData(roDataEcuHwVer, mECUVer.OemHW);
                            mDBX.SetData(roDataEcuSwVer, mECUVer.OemSW);

                            mDBX.SetData(roRsEcuPartNo, mEVItem.MatchPartNo(mECUVer.PartNo));
                            mDBX.SetData(roRsEcuPartNo, mEVItem.MatchHWVer(mECUVer.OemHW));
                            mDBX.SetData(roRsEcuPartNo, mEVItem.MatchSWVer(mECUVer.OemSW));
                    ...
                    1:
                        begin
                            ...
                            begin
                                ...
                                mDBX.SetData(roRsLimit, mIMSCtrler.GetLimit(MtrIt));
                            end;
                            UpdateForm(tw_LIMIT_READED);
        ...
        _WS_WEIGHT_UNLOADING:
            begin
                ...
                begin
                    ...
                    gLog.Panel('%s: 웨이트 언로딩 완료, 납품 위치로 이동 시작', [Name]);
    ...
    TAsyncCalls.Invoke(
        ...
        begin
            ...
            try
                ...
                begin
                    gLog.Panel('%s: 측정 중 NG: 시험 정지', [Name]);
                    mIsTestingNG := True;
                    mFuncState := 0;
                    mWorkState := _WS_MOVE_TO_UNLOADING_POS;
                end;
...
begin
    Result := mAD.GetValue(AI_DISTANCE1);
end;
...
const
    MEM_TEST_DELAY = 500;
    MEM_MOVE_TIME = 2.0;          // 메모리 위치 이동 시간
    MEM_CURR_THRESHOLD = 1.0;     // 동작 감지용 전류 임계값
    ...
    case mMemTestState of
        _MEM_INIT:
            begin
                ...
                begin
                    ...
                    gLog.Panel('%s: IMS 사양 아님 메모리 테스트 안함', [Name]);
                end;
            end;
        ...
        _MEM_PLAY_MEM1:
            begin
                ...
                // MEM1 재생
                case mIMSCtrler.FSMPlayMemP1 of
                    -1:
                        begin
                            gLog.Error('%s: MEM1 재생 실패', [Name]);
                            mMemTestState := _MEM_ERROR;
                        end;
                    1:
                        begin
                            // 전류 모니터링 시작
                            mMMA.Clear;
                            mTC.Start(MEM_CURR_CHECK_TIME * 1000);
                            mMemTestState := _MEM_CHECK_MEM1_CURR;
                            gLog.Panel('%s: MEM1 재생 시작 - 전류 모니터링', [Name]);
                        end;
                end;
            end;

        _MEM_CHECK_MEM1_CURR:
            begin
                // 전류 모니터링
                CurrVal := GetMotorCurr(tmSlide);
                mMMA.Add(CurrVal);
                ...
                begin
                    // 전류가 올랐다가 떨어짐 = 동작 완료
                    mMemTestState := _MEM_JUDGE_MEM1;

                    gLog.Panel('%s: MEM1 동작 완료 감지 - 최대전류: %.2f A', [Name, mMMA.mMax]);
                end
                else if mTC.IsTimeOut then
                begin
                    // 타임아웃
                    mMemTestState := _MEM_JUDGE_MEM1;
                    gLog.Panel('%s: MEM1 전류 체크 타임아웃 - 최대전류: %.2f A', [Name, mMMA.mMax]);
                end;
            end;
        ...
        _MEM_JUDGE_MEM1:
            begin
                ...
                begin
                    ...
                    gLog.Panel('%s: MEM1 동작 OK - 최대전류: %.2f A', [Name, mMMA.mMax]);
                ...
                begin
                    ...
                    gLog.Panel('%s: MEM1 동작 NG - 최대전류: %.2f A', [Name, mMMA.mMax]);
                end;
        ...
        _MEM_PLAY_MEM2:
            begin
                ...
                // MEM2 재생
                case mIMSCtrler.FSMPlayMemP2 of
                    -1:
                        begin
                            gLog.Error('%s: MEM2 재생 실패', [Name]);
                            mMemTestState := _MEM_ERROR;
                        end;
                    1:
                        begin
                            ...
                            mMMA.Clear;
                            mTC.Start(MEM_CURR_CHECK_TIME * 1000);
                            mMemTestState := _MEM_CHECK_MEM2_CURR;
                            gLog.Panel('%s: MEM2 재생 시작 - 전류 모니터링', [Name]);
                        end;
                end;
            end;

        _MEM_CHECK_MEM2_CURR:
            begin
                // 전류 모니터링
                CurrVal := GetMotorCurr(tmSlide);
                mMMA.Add(CurrVal);
                ...
                begin
                    // 전류가 올랐다가 떨어짐 = 동작 완료
                    mMemTestState := _MEM_JUDGE_MEM2;
                    gLog.Panel('%s: MEM2 동작 완료 감지 - 최대전류: %.2f A', [Name, mMMA.mMax]);
                end
                else if mTC.IsTimeOut then
                begin
                    // 타임아웃
                    mMemTestState := _MEM_JUDGE_MEM2;
                    gLog.Panel('%s: MEM2 전류 체크 타임아웃 - 최대전류: %.2f A', [Name, mMMA.mMax]);
                end;
            end;
        ...
        _MEM_JUDGE_MEM2:
            begin
                ...
                begin
                    ...
                    gLog.Panel('%s: MEM2 동작 OK - 최대전류: %.2f A', [Name, mMMA.mMax]);
                ...
                begin
                    ...
                    gLog.Panel('%s: MEM2 동작 NG - 최대전류: %.2f A', [Name, mMMA.mMax]);
                end;
                UpdateForm(tw_MEMORY_END);
