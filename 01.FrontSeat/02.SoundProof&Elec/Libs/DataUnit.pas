...
{==============================================================================}
    ...
    TMotorSpec = packed record  // sizeof=100
        ...
        rSpeed, rPos: array[TMotorDir] of TSpec; // 속도, 또는 위치..


        rInitNoiseTime, rInitNoise,        // 초기 소음 Max (초기.)
        rRunNoise, rInitNoiseDev, rRunNoiseDev: Single;              // 구동 소음
    ...
    TSpecs = packed record
        ...
        rBuckleCurr: TSpec;
        rAncPTReg: TSpec; // 앵커PT 규제

        rREM: array[0..50] of BYTE;
    ...
    // 모터 구동량  속도 제한, 시간, 거리, 전류량
    PMotorConstraints = ^TMotorConstraints;

    TMotorConstraints = packed record
        rMaxTime: Single; // 구동 최대 시간
        rStroke: Single; // 속도 제어시 최대 구동 거리.
...
{==============================================================================}
    ...
    TModel = packed record // sizeof = 1000
        rIndex: WORD;       // Spec Index를 의미

        rTypes: TModelType;

        rPartName: string[50];
        rPartNo: string[30];
        rLclPartNo: string[30];     // Local PartNo : ex> 대원산업 전체 품번

        rSpecs: TSpecs;

        rSPITypeBits: DWORD;        // Seat Pin Info TypeBits: 시트 핀 정보 ID(Key)활용
...
{==============================================================================}
    ...
    TMotorTestItem = packed record
        rTime, rCurr, rSpeed, rInitNoise, rRunNoise, rInitNoiseDev, rRunNoiseDev: Single;  // 결과
    ...
    TResult = packed record // sizeof = ??
        ...
        rValidity: array[TTsORD] of Boolean; // 센서 유/무 검사
        rExists: array[TTsORD] of Boolean; // 데이터 유/무.
        rTested: array[TTsORD] of Boolean; // 검사여부
        rFileTime: Double; // 파일작업시간을 의미하는 시간 + 작업시간
        rLastTime: Double; // 마지막작업 + 작업시간. -> Lot No 의미
        ...
        rBuckleVal,             // 버클 값(전류)
        rAncPTVal: Double;        // 앵커 PT


        rDatNoise: Double;      // 검사 시간을 의미 체크(옵션)
    ...
    // ------------------
    TFaGRAPH_ORD = (faSlide, faSlideNoise, faTilt, faTiltNoise, faHeight, faHeightNoise, faLegSupt, faLegSuptNoise, faSwivel, faSwivelNoise);
    ...
    end;    // record 마지막에 end 하나 더 !!!   , rTime, rMin, rDist는 실제 이름 다름

    TMotorDeliveryPos = packed record // sizeof 350
        rTypeBits: DWORD;
        rMotors: array[False..True, 0..3] of TDeliveryPos;
        rMEM2: array[0..2] of TDeliveryPos; // 2013.06.20 전,후진시 제품위치를 메모리 위치에 저장.
    ...
    TUsrCount = packed record
        rTime: Double;
        rCount: array[0..1, False..True] of Integer; // 작업번호,합/불
    end;
...
{==============================================================================}
    ...
    TModels = packed record // sizeof = ??
        ...
        rMtrOffset: array[0..MAX_ST_COUNT - 1] of TMotorOffset; // 모터 오프셋
        rOffset: array[0..MAX_ST_COUNT - 1] of TOffset; // 옵션 오프셋


        rOutPos: array[TMotorOrd] of TOutPos;  // 출하 위치

        rRearSensorIdx: byte;            // 후방 센서 사용 Idx

        rDistance: Single;

        rREM: array[0..936] of BYTE;

    end;

    TOutPosInfo = packed record
        mType: Integer;                // 구분 : 전진? 후진?
        mSensorIdx: Integer;                // 센서 사용 센서 Idx : 0:#13 1:#14 2: 둘다
        mSpec: array[0..1] of TRange;           // 제품위치 센서별 거리 SPEC
        mDirs: array[TMotorOrd] of TMotorDir;     // 모터별 사용 제품 위치
...
//    OK_TAG = '';
    ...
    GRP_EXT: array[ord(Low(TFaGRAPH_ORD))..ord(High(TFaGRAPH_ORD))] of string = ('Slide Curr', 'Slide Noise', 'Tilt Curr', 'Tilt Noise', 'Height Curr', 'Height Noise', 'Leg Support Curr', 'Leg Support Noise', 'Swivel Curr', 'Swivel Noise');
    ...
    DataGridORD: array[0..110] of TResultORD = (roIndex, roDate, roTime, roPartNo, roPartName, roCarType, roLotNo, roMcNo, roNo,
        ...
        roDataBwSpeed,        // 후진 속도
        rospecInitNoiseHi,  // 초기 소음 스펙 상한
        roDataFwInitNoise,    // 전진 초기 소음
        roDataBwInitNoise,    // 후진 초기 소음
        rospecRunNoiseHi,   // 구동 소음 스펙
        roDataFwRunNoise,     // 전진 구동 소음
        roDataBwRunNoise,     // 후진 구동 소음
...
procedure MakeDataCtrlOrdFromDataGridOrd(var DataCtrlORD: array of TResultORD; const Target, Replace: array of TResultORD);
...
var
    ...
    // 현재 모드 상태-> 1= 검사한다, 2=완료
    // 현재 하나는 0 이고 나머지는 2이다, 즉 2이다.
...
begin
    ...
    begin
        ...
        gLog.ToFiles('GraphEnv File 크기이상 에러.', []);
        goto _INIT_GRP;
    end;
...
var
    i, j: Integer;
    CurrentReplace: TResultORD;
begin
    j := 0;
    CurrentReplace := roNone;

    for i := Low(DataCtrlORD) to High(DataCtrlORD) do
    begin
        if (j <= High(Target)) and (DataGridOrd[i] = Target[j]) then
        begin
            if j <= High(Replace) then
                CurrentReplace := Replace[j]
            else
                CurrentReplace := roNone;
            Inc(j);
        end;

        DataCtrlORD[i] := CurrentReplace;
    end;
end;
