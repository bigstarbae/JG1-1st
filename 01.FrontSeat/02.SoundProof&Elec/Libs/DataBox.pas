...
uses
    Math, ModelUnit, Log, DataUnitHelper, SysEnv, SubDataBox4Mtr, SubDataBox4IMS, SubDataBox4Elec, LangTran;
...
begin
    ...
    gModels.CurrentOffset(TStationID(mTag), mOffset, mMotorOffset);
...
begin
    ...
    begin
        ...
        rValidity[tsHeight] := GetResult(tsHeight);
        rValidity[tsLegSupt] := GetResult(tsLegSupt);
        rValidity[tsSwivel] := GetResult(tsSwivel);
...
begin
    ...
    case APos of
        ord(roNone):
            Exit;
        ord(roIndex), ord(roPosType), ord(roPosExType), ord(roDrvType), ord(roWayType), ord(roSeatOpType), ord(roImsType), ord(roCarType), ord(roPartName), ord(roPartNo),
          ord(roLclPartNo):
            Result := GetModelToTxt(mBuf.rModel, ord(APos), IsUnit);

        ord(roTestType):
            begin
                if mBuf.rTested[tsWeight] then
                    Result := _TR('부하')
                else
                    Result := _TR('무부하');
            end;
...
begin
    ...
    case AORD of
        ...
        roNO:
            begin
                 Result := GetCompoJudge([roRsTotMotor, roRsAbnormalSound, roRsElec, roRsIMS]);
            end;
    else
