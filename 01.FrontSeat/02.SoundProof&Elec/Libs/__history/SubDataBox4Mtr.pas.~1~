...
begin
    ...
    begin
        case AORD of
            ...
            roRsNoise:
                begin
                    Result := GetResult(roDataFwInitNoise) and GetResult(roDataFwRunNoise) and
                              GetResult(roDataBwInitNoise) and GetResult(roDataBwRunNoise);
                end;
            ...
            roRsFwMotor:
                begin
                    Result := GetResult(roDataFwInitNoise) and GetResult(roDataFwRunNoise) and
                              GetResult(roDataFwSpeed) and GetResult(roDataFwCurr);
                end;
            roRsBwMotor:
                begin
                    Result := GetResult(roDataBwInitNoise) and GetResult(roDataBwRunNoise) and
                              GetResult(roDataBwSpeed) and GetResult(roDataBwCurr);
                end;
...
begin
    ...
    begin
        case APos of
            ...
            ord(roRsFwMotor), ord(roRsBwMotor),
            ord(roRsTotCurr), ord(roRsMotor), ord(roRsTotMotor), ord(roRsNoise), ord(roRsTotNoise), ord(roRsAbnormalSound):
                begin
                    if not IsExists(TResultORD(APos)) and not IsTested(TResultORD(APos)) then
                        Exit;
...
begin
    ...
    begin
        ...
        case AORD of
            ...
            roDataFwCurr, roDataBwCurr, roDataFwSpeed, roDataBwSpeed:
            ...
            roNo:
                begin
                    Result := IsExists(tsSlide) or IsExists(tsTilt) or
                        IsExists(tsHeight) or IsExists(tsLegSupt) or IsExists(tsSwivel) or
                        IsExists(tsNoise);
                end
        else
            mIsProcessDone := False;
        ...
        case AORD of
            ...
            roRsTotMotor:
                Result := IsTested(tsSlide) or IsTested(tsTilt) or IsTested(tsHeight) or
                    IsTested(tsLegSupt) or IsTested(tsSwivel);
