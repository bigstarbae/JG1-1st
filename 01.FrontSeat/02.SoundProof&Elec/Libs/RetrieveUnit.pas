...
uses
  ...
  Graphics, DataUnit, DataUnitOrd, BaseDataBox, GridHead, RetrieveEx, XbarRLib, SelGrid ;
...
public
    ...
    function  GetAData(RecNo, ROCol:Integer; aData: TResultORD; IsUnit:boolean=false): string ;
    ...
    procedure SetGridORD(AORD: array of TResultORD) ; overload;
    procedure SetGridORD(AORD, CtrlOrd: array of TResultORD) ; overload;
...
begin
    ...
    mBegin  := Trunc(Now) + Frac(gsysEnv.rOP.rWorkTime) ;
...
procedure TRetrieve.SetGridORD(AORD, CtrlOrd: array of TResultORD);
begin
    SetGridOrd(AOrd);
    SetLength(mCtrlOrd, Length(CtrlOrd));

    Move(CtrlOrd[0], mCtrlOrd[0], Length(CtrlOrd) * SizeOf(TResultOrd));
end;
...
procedure TRetrieve.DrawCell(Sender: TObject; ACol, ARow: Integer; Rect: TRect; State: TGridDrawState);
var
    asData : string ;
...
begin
	...
	begin
        ...
        if Length(mGridORD) > 0 then asData := GetAData(ARow, ACol, mGridORD[ACol], True)
        ...
        //-- DATA들
        //====
        if Assigned(mUsrDrawCell) then
            OnUserDrawCell(Sender,ACol, ARow,Rect,State) ;
        //====
...
begin
    ...
    begin
        aDatas.Add(GetAData(aReadPos, i, mGridORD[i]));//DataGridORD[i])) ;
    end ;
end ;
...
function  TRetrieve.GetAData(RecNo, ROCol:integer; AData: TResultORD; IsUnit:boolean): string ;
begin
    Result := '' ;
    if (mNG+mOK) <= 0 then Exit;
    if mLstIDx <> RecNo then
    begin
        if not GetATemp(mTmpBuf[0], RecNo) then
        begin
            mLstIDx := -1 ;
            Exit ;
        end ;
    ...
    if Length(mCtrlOrd) > 0 then // CTRL ResultOrd로 자료구조 Idx제어
    begin
        if mCtrlOrd[ROCol] <> roNone then
            mBox.GetResultToATxt(Ord(mCtrlOrd[ROCol]), False, False);
    end;

    if aData = roIndex then
        Result := IntToStr(RecNo)
    else
        Result := mBox.GetResultToATxt(ord(AData),IsUnit,true) ;
end ;
...
begin
    ...
    begin
        ...
        begin
            GetAData(mTmpAry[ARow-1], ACol, mGridORD[0]);
        end ;
