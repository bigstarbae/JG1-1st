...
uses
    ...
    LanIoUnit, DataUnit,  DataUnitOrd, Generics.Collections, ModelType, SeatType, BlckSock, TimeChecker;
{
사양정보
구분자	"@"
0	STX
...
begin
    ...
    try
        Ini.WriteString(sTm, 'HostIp', mClient.IP) ;
        Ini.WriteInteger(sTm, 'Port', mClient.Port) ;
        Ini.WriteInteger(sTm, 'ConnectTime', mConnectTime) ;
        Ini.WriteInteger(sTm, 'SendTerm', mSendTerm) ;
        Ini.WriteBool(sTm, 'ReConnect', mReConOfSndFault) ;
        Ini.WriteBool(sTm, 'Use', mUsePop) ;
        Ini.WriteInteger(sTm, '_POP 응답대기시간', mResponseTime) ;
        Ini.WriteBool(sTm, 'SndData', mNotsndResult) ;
    finally
        Ini.Free ;
    end ;
end;
