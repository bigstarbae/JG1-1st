...
implementation
uses
    ModelUnit, Log, DataUnit, myUtils, KiiMessages, SelModelForm, CustomOpenDialog, LangTran ;
...
procedure TfrmUserModel.btnExportClick(Sender: TObject);
begin
    if sdlgSave.Execute() then
    begin
        gModels.SaveToIni(sdlgSave.FileName);
    end;
end;

procedure TfrmUserModel.btnImportClick(Sender: TObject);
var
    cdlgOpen : TCustomOpenDialog;
    CondBits : BYTE;
begin

    try
        cdlgOpen := TCustomOpenDialog.Create(Self);
        cdlgOpen.SetSize(0, pnlImpOpt.Height);
        cdlgOpen.AddControl(pnlImpOpt);
        cdlgOpen.Filter := 'ini|*.ini|env|*.env';

        if cdlgOpen.Execute() then
        begin
            CondBits := 0;

            if cbPartName.Checked then CondBits := BITON(CondBits, 0);
            if cbPartNo.Checked then CondBits := BITON(CondBits, 1);
            if cbTypes.Checked then CondBits := BITON(CondBits, 2);
            if cbSpecChk.Checked then CondBits := BITON(CondBits, 3);

            if (Pos('env', ExtractFileExt(cdlgOpen.FileName)) > 0) then
                gModels.LoadFromEnv(cdlgOpen.FileName, cbOverWrite.Checked, CondBits)
            else
                gModels.LoadFromIni(cdlgOpen.FileName, cbOverWrite.Checked, CondBits);

            ListUpdates;
    //        with mEditor do SetFrm(mIdx) ;

        end;

    finally
        //cdlgOpen.RemoveControl(pnlImpOpt);
        cdlgOpen.Free;
    end;
end;
...
begin
    if Assigned(mEditor) then mEditor.Close ;
    mEditor := nil ;
end;
...
begin
    ListUpdates(true) ;
    if not Assigned(gModels) then exit ;
...
begin
    nNode := nil ;
    if AInit then tvList.Items.Clear ;
    if not Assigned(gModels) then exit ;
    ...
    begin
        ...
        begin
            ...
            begin
                if AInit then nNode := Add(nNode, sTm)
                else
                begin
                    if not Assigned(nNode) then nNode := GetFirstNode
                    else                        nNode := nNode.GetNext ;
...
procedure TfrmUserModel.pnlTitleMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
    ReleaseCapture() ;
...
var
    ...
    nB : array of boolean ;
begin
    if not Assigned(mEditor) then Exit ;
    ...
    if not frmSelModels.Execute(nP.X, nP.Y) then Exit ;
    ...
    mEditor.Save(nB,
                 frmSelModels.ckbApplyMotor.Checked,
                 frmSelModels.ckbAppyDelivery.Checked,
                 frmSelModels.ckbAppySwIo.Checked,
                 frmSelModels.ckbUsrType.Checked,
                false) ;
    ListUpdates ;
    if Length(nB) > 0 then SetLength(nB, 0) ;
end;
...
begin
    ...
    mEditor.Save([],false,false,false,false, false) ;
    ListUpdates ;
end;
