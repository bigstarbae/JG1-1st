...
uses
    ...
    Grids, ExtCtrls, ComCtrls, StdCtrls, Buttons, CheckLst, RetrieveUnit,
    ModelType, SeatType, Menus, KiiMessages;
...
type
    ...
    TfrmRetrieve = class(TForm)
        ...
        procedure sgrdResultExDrawCell(Sender: TObject; ACol, ARow: Integer; Rect: TRect; State: TGridDrawState);
...
uses
    ...
    SelGrid, RetrieveEx, IniFiles, ShellApi, DateUtils, Global, KiiFaGraphDB,
    ToExcelUnit, BIFFRecsII4, CellFormats4, PasswdForm, shlObj, VerUnit,
    DetailForm, SysEnv, GridHead, WaitForm, LangTran;
...
begin
    with GHeader do
    begin
        AddHeader(['순번, 		작업 정보, 	#L,        #L,   	  #L,      #L,     #L,     공정, 전체 결과', '#T,   	작업 일자, 	작업 시간, PART NO, PART NAME, 차종,   LOT NO, #T,   #T', '#T,		#T,			#T,		   #T,		  #T,      #T,     #T,     #T,   #T']);

        AddHeader(['이음', '#T', '#T']);

        AddHeader(['ECU INFO.,  #L,     #L', 'PART NO,    S/W,    H/W', '#T,         #T,     #T']);

        AddHeader(['IMS Test,  #L,         #L,         #L', '메모리,    #L,         #L,         EasyAccess', '1,         2,          3,          #T']);

        AddHeader(['DTC Clear', '#T', '#T']);

        AddHeader(['Anchor PT,     #L,         #L', '스펙값,        측정값,     결과', '#T,            #T,         #T']);

        AddHeader(['Buckle,        #L,         #L', '스펙값,        측정값,     결과', '#T,            #T,         #T']);

        AddHeader(['Slide,       #L,      #L,          #L,      #L,          #L,      #L,      #L,          #L,      #L,     #L,           #L,      #L,    #L', '속도 검사,   #L,      #L,          #L,      초기 소음,   #L,      #L,      작동 소음,   #L,      #L,     전류,         #L,      #L,    Limit', '검사 기준,   전진,    검사 기준,   후진,    검사 기준,   전진,    후진,    검사 기준,   전진,    후진,   검사 기준,    전진,    후진,  #T']);

        AddHeader(['Tilt,       #L,      #L,          #L,      #L,          #L,      #L,      #L,          #L,      #L,     #L,           #L,       #L,    #L', '속도 검사,   #L,      #L,          #L,      초기 소음,   #L,      #L,      작동 소음,   #L,      #L,     전류,         #L,      #L,    Limit', '검사 기준,   전진,    검사 기준,   후진,    검사 기준,   전진,    후진,    검사 기준,   전진,    후진,   검사 기준,    전진,    후진,  #T']);

        AddHeader(['Height,       #L,      #L,          #L,      #L,          #L,      #L,      #L,          #L,      #L,     #L,          #L,      #L,    #L', '속도 검사,   #L,      #L,          #L,      초기 소음,   #L,      #L,      작동 소음,   #L,      #L,     전류,         #L,      #L,    Limit', '검사 기준,   전진,    검사 기준,   후진,    검사 기준,   전진,    후진,    검사 기준,   전진,    후진,   검사 기준,    전진,    후진,  #T']);

        AddHeader(['LegSupt,     #L,      #L,          #L,      #L,          #L,      #L,      #L,          #L,      #L,     #L,           #L,      #L,    #L', '속도 검사,   #L,      #L,          #L,      초기 소음,   #L,      #L,      작동 소음,   #L,      #L,     전류,         #L,      #L,    Limit', '검사 기준,   전진,    검사 기준,   후진,    검사 기준,   전진,    후진,    검사 기준,   전진,    후진,   검사 기준,    전진,    후진,  #T']);

        AddHeader(['Swivel,       #L,      #L,          #L,      #L,          #L,      #L,      #L,          #L,      #L,     #L,           #L,     #L,    #L', '속도 검사,   #L,      #L,          #L,      초기 소음,   #L,      #L,      작동 소음,   #L,      #L,     전류,         #L,      #L,    Limit', '검사 기준,   전진,    검사 기준,   후진,    검사 기준,   전진,    후진,    검사 기준,   전진,    후진,   검사 기준,    전진,    후진,  #T']);

        AddHeader(['히터 검사,     #L,         #L,         #L,         #L,     #L,     #L,     #L', 'On 스펙,       On 측정,    Off 스펙,   Off 측정,   LED,    #L,     #L,     #L', '#T,            #T,         #T,         #T,         3단,    2단,    1단,    OFF']);

        AddHeader(['통풍 검사,     #L,         #L,         #L,         #L,     #L,     #L,     #L,     #L', 'On 스펙,       On 측정,    Off 스펙,   Off 측정,   LED,    #L,     #L,     #L,     통풍센서', '#T,            #T,         #T,         #T,         3단,    2단,    1단,    OFF,    #T']);
...
begin
    ...
    mRetrieve := TRetrieve.Create(sgrdResult, CreateHeader(sgrdResult), TDataBox.Create(0), nil, PROJECT_NAME, _ITEM_COUNT, sizeof(TResult), 0);

    SetLength(DataCtrlOrd, sgrdResult.ColCount);
    MakeDataCtrlOrdFromDataGridOrd(DataCtrlOrd, [rospecFwSpeedHiLo, rospecFwSpeedHiLo, rospecFwSpeedHiLo, rospecFwSpeedHiLo, rospecFwSpeedHiLo, rospecHeatOnHiLo, rospecVentOnHiLo], [roSlide_CTRL, roTilt_CTRL, roHeight_CTRL, roLegSupt_CTRL, roSwivel_CTRL, roHVDrvHtr_CTRL, roHVDrvVnt_CTRL]);
...
procedure TfrmRetrieve.sgrdResultExDrawCell(Sender: TObject; ACol, ARow: Integer; Rect: TRect; State: TGridDrawState);
begin

    AlignGrid(TStringGrid(Sender), ACol, ARow, Rect, State, [TA_CENTER, TA_CENTER, TA_RIGHT, TA_RIGHT, TA_RIGHT, TA_RIGHT, TA_RIGHT, TA_RIGHT, TA_RIGHT, TA_RIGHT, TA_RIGHT, TA_RIGHT, TA_RIGHT, TA_RIGHT, TA_RIGHT]);
end;
...
var
    ...
    Result: TResult;
begin
    if ((mRetrieve.OK + mRetrieve.NG) <= 0) and not Button1.Visible then
        Exit;
...
const
    RESULT_TXT: array[0..11] of string = ('No', 'Part Name', '작업수량', '양품', '불량', 'Slide', 'Tilt', 'Height', 'LegtSupt', 'Swivel', '소음', '이음');
...
begin
    ...
    begin
        with Application do
            MessageBox(PChar(_TR('검색일자를 조정하십시요!')), PChar(Caption), MB_ICONWARNING + MB_OK);
        Exit;
    end;
...
begin
    //선택된것만 할건지 물어보자.
    if mRetrieve.GetSelGrid = nil then
        Exit;
    with Application do
        if MessageBox(PChar(_TR('검사 작업중일 경우 작업에 영향을 줄 수 있습니다, 계속하시겠습니까?')), PChar(_TR('엑셀 변환!')), MB_ICONWARNING + MB_YESNO) = IDNO then
        begin
            Exit;
        end;
    ...
    begin
        with Application do
            MessageBox(PChar(_TR('변환할 데이터가 너무 많습니다, 변환 범위를 줄여주십시요(<60000)')), PChar(_TR('엑셀 변환!')), MB_ICONWARNING);
    ...
    begin
        ...
        begin
            ...
            begin
                with Application do
                    MessageBox(PChar(_TR('지정된 파일이 사용중입니다, 파일을 닫은 후 다시 작업하세요!')), PChar(_TR('엑셀 변환!')), MB_ICONERROR + MB_OK);
                Exit;
            end;
            //----------
    ...
    if Length(exlData) > 0 then
        SetLength(exlData, 0);
    if sbtnUnSelect.Enabled then
        sbtnUnSelect.Click;
...
begin
    if (mRetrieve.OK + mRetrieve.NG) <= 0 then
        Exit;
...
procedure TfrmRetrieve.FormClose(Sender: TObject; var Action: TCloseAction);
begin
    {$IFNDEF _VIEWER}
    Action := caFree;
    {$ENDIF}
...
procedure TfrmRetrieve.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
    with TfrmWait.Create(Application) do
    begin
...
procedure TfrmRetrieve.OnUsrSearchDatas(Sender: Tobject; var Buf; var aPass, aResult: boolean);
var
    nRs: TResult;
    aVal: integer;
begin
    ...
    begin
        ...
        // date check
        // a day Time check
        if (dtpStart.DateTime > nRs.rLastTime) or (dtpEnd.DateTime < nRs.rLastTime) then
            Exit;

        if lpList.Count > 0 then
        begin
            if not IsTxtInList(lpList, nRs.rModel.rPartName) and not IsTxtInList(lpList, nRs.rModel.rPartNO)
                //and not IsTxtInList(lpList, nRs.rModel.rTypes.GetOpTypeStr)
                and not IsTxtInList(lpList, nRs.rModel.rTypes.GetPosStr) and not IsTxtInList(lpList, nRs.rLotNo) then
                Exit;
        end;

        if ckbOk.Checked <> ckbNG.Checked then
        begin
            if (nRs.rValidity[tsResult] <> ckbOk.Checked) and (not nRs.rValidity[tsResult] <> ckbNG.Checked) then
                Exit;
        end;
        ...
        begin
            ...
            if (aVal = BYTE(ckbNonIMS.Checked)) or (aVal <> BYTE(ckbIMS.Checked)) then
                Exit;
        end;
        ...
        begin
            ...
            if (lpBox.IsTested(tsLegSupt) and not lpBox.GetResult(tsLegSupt)) then
            begin
                AddData(nRs.rModel.rPartName, false, 4);
            end;

            if (lpBox.IsTested(tsSwivel) and not lpBox.GetResult(tsSwivel)) then
            begin
                AddData(nRs.rModel.rPartName, false, 5);
            end;

            if (lpBox.IsTested(tsNOISE) and not lpBox.GetResult(roRsTotNoise)) then
            begin
                AddData(nRs.rModel.rPartName, false, 6);
            end;

            if lpBox.IsTested(roRsAbnormalSound) and not lpBox.GetResult(roRsAbnormalSound) then
            begin
                AddData(nRs.rModel.rPartName, false, 7);
            end;
...
procedure TfrmRetrieve.UpdateItems(const AList: array of WORD; ACount: integer);
var
    i, fh: integer;
    Buf: TResult;
begin

    if not Assigned(lpItems) then
        Exit;
    with lpItems do
    begin
        Initial;
...
procedure TfrmRetrieve.OnUsrListToExcelAfter(Sender: TObject; var AePos: integer);
var
    i, j: integer;
    sTm: string;
begin
...
procedure TfrmRetrieve.OnUsrListToExcelBefore(Sender: TObject; var AbPos: integer);
var
    i, j: integer;
    CellRect: TRect;
begin
...
begin
    ...
    if Buf1.rLastTime > Buf2.rLastTime then
        Result := 1
    else if Buf1.rLastTime < Buf2.rLastTime then
        Result := -1
    else
        Result := 0;
end;
...
begin
    ...
    if (First < i - 1) then
        DataQuickpSort(AfHandle, First, i - 1);
    if (j + 1 < Last) then
        DataQuickpSort(AfHandle, j + 1, Last);
end;
...
var
    fh: integer;
begin
    if not FileExists(mRetrieve.TmpFile) or not ckbSort.Checked then
        Exit;
...
initialization
    ...
    begin
        with Application do
            MessageBox(PChar(PROJECT_NAME + _TR(' 조회 프로그램이 실행중입니다.')), PChar(PROJECT_NAME + _TR(' 조회 프로그램')), MB_ICONSTOP + MB_OK);
