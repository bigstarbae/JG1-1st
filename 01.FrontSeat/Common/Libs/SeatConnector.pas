...
type
    ...
    public
        ...
        function IsSwType: Boolean; virtual; abstract;             // 스위치 타입 커넥터?
        ...
        function IsTarVoltPin(PinNo: Integer): Boolean;        // TarVolt  50% 이상 ON으로 간주
        function IsVoltOnPin(PinNo: Integer): Boolean;         // 커넥터 부착시 자체 전압 출력 핀, 2V이상 ON으로 간주
    ...
    TSeatConnector = class(TBaseSeatConnector)
    public
        function IsConnected: Boolean; override;     //
        function IsSwType: Boolean; override;             // 스위치 타입 커넥터?

        procedure ShowConDebugLog;
    end;
...
function TSeatConnector.IsConnected: Boolean;
begin
    if mSpecificPinNos.Count = 0 then
        Exit(False);
    ...
    Result := IsTarVoltPin(GetSpecificPinNos('RECL_FWD_PIN')[0]) and IsTarVoltPin(GetSpecificPinNos('RECL_BWD_PIN')[0]);

   // Result := GetSpecificPinNoVolt('RECL_BWD_PIN', 0) >= (mTarVolt * 0.5);
end;

function TSeatConnector.IsSwType: Boolean;
begin
    Result := False;
end;

procedure TSeatConnector.ShowConDebugLog;
begin
    gLog.Debug('FWD:%d(%f) %s , BWD:%d(%f) %s', [GetSpecificPinNos('RECL_FWD_PIN')[0], GetPinVolt(GetSpecificPinNos('RECL_FWD_PIN')[0]), BoolToStr(IsTarVoltPin(GetSpecificPinNos('RECL_FWD_PIN')[0]), True), GetSpecificPinNos('RECL_BWD_PIN')[0], GetPinVolt(GetSpecificPinNos('RECL_BWD_PIN')[0]), BoolToStr(IsTarVoltPin(GetSpecificPinNos('RECL_BWD_PIN')[0]), True)]);
end;
...
function TSwConnector.IsSwType: Boolean;
begin
    Result := True;
end;
...
begin
  ...
  // 검색할 ID 정규화 (공백 제거, 대문자 변환)
    ...
    begin
        Iter := Connectors[i];
        NormalizedName := StringReplace(Iter.S['ID'], ' ', '', [rfReplaceAll]);
        if SameText(NormalizedName, ID) then
            Exit(Iter);
    end;
end;
