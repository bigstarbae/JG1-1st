...
private
    ...
    {$IFDEF USEPWS_CURR}
    mMeasCurr: double;   // [수정] 측정된 전류(ReadBack) 값을 저장할 변수
    {$ENDIF}
    ...
    procedure SetUsrLog(const szFmt : string; const Args: array of const;
...
public
    ...
    {$IFDEF USEPWS_CURR}
    property  MeasCurr: double read mMeasCurr; // [수정] 외부에서 측정된 전류 값을 읽기 위한 속성
    {$ENDIF}
...
const
    ...
    _ORD_CURR  = 2 ;
    {$IFDEF USEPWS_CURR}
    _ORD_MEAS_CURR = 3; // [수정] 측정 전류 읽기 상태 추가
    {$ENDIF}
...
begin
    mLogHead:= Format('%s(COM%d)',
        [GetDevComName(ADevCom.DevID), ADevCom.Port]) ;
    gLog.ToFiles('create',[]);
...
begin
    ...
    if not FileExists(sTm) then SaveEnv ;
end ;
...
begin
    ...
    begin
        SetErrorTxt(Format('%s Power Supply', [GetDevComName(mDevCom.DevID)]),
                    Format('COM%d %s', [mDevCom.Port, _TR('통신포트 열기실패')]),
                    _TR('통신포트설정 확인 하세요.'),
                    umtError);
...
begin
    Result:= '' ;
    if Assigned(mDevCom)
        and not mDevCom.Lock
        and (mDevCom.GetInQue > 0) then
    begin
        mDevCom.Lock:= true ;
    ...
    Buf:= '' ;
    if (Length(Trim(mrcvBuf)) <= 0) or (Pos(LF, mrcvBuf) <= 0) then Exit ;
...
begin
    ...
    case msubProc of
      ...
      _ORD_CURR:  // [수정] else 문을 명시적인 case문으로 변경하여 가독성 및 안정성 향상
      begin
          mCurr:= StrToFloatDef(sTm, 0) ;
      end;
      {$IFDEF USEPWS_CURR}
      _ORD_MEAS_CURR: // [수정] MEAS:CURR? 응답 처리
      begin
...
begin
    ...
    case msubProc of
        _ORD_POWER:
        begin
            if not IsWrite then Result:= ReqOutPower()
            else
            if mSetPower then PowerON(true)
            else              PowerOFF(true) ;
        end ;
        _ORD_VOLT:
        begin
            if not IsWrite then Result:= ReqVolt()
            else                SetVolt(mSetVolt,true);
        end ;
        _ORD_CURR:
        begin
            if not IsWrite then Result:= ReqCurr()
            else                SetCurr(mSetCurr,true);
        end ;
        {$IFDEF USEPWS_CURR}
        _ORD_MEAS_CURR: // [수정] 측정 전류 요청 명령 전송
        begin
          if not IsWrite then Result := SendToUnit('MEAS:CURR?');
        end;
        {$ENDIF}
    else
        Result:= false ;
...
begin
    ...
    if Result and gPwrDebugMode then SetUsrLog('snd: %s',[sTm]);
end;
...
procedure TDcPower.SetUsrLog(const szFmt : string; const Args: array of const;
    IsFile:boolean);
begin
    if not IsFile then gLog.Panel(mLogHead+' '+szFmt, Args)
    else               gLog.ToFiles(mLogHead+' '+szFmt, Args);

    if mLogHandle <= 32 then Exit ;
...
begin
    ...
    case mProc of
        ...
        1:
        begin
            if not mDevCom.IsOpen
                and not mDevCom.Open then
            begin
       ...
       //   gLog.Panel('PROC = %d, %s',[mProc, GetDevComName(mDevCom.DevID)]);
        ...
        begin
            ...
            if (GetAccurateTime - mTmpTime) >= (gsysEnv.rComm.rWaitResPwrSply * 1000.0) then
            begin
                if mRepeat > 3 then
                begin
                    ...
                    begin
                        ...
                        SetErrorTxt(Format('%s Power Supply', [GetDevComName(mDevCom.DevID)]),
                                    _TR('응답이 없습니다.!'),
                                    _TR('파워서플라이 전원 및 케이블 연결, 통신포트설정을 확인 하세요.'),
                                    umtError);
                    end ;
        ...
        4:
        begin
            // [수정] 주기 변경 및 USEPWS_CURR에 따른 분기 처리
            {$IFDEF USEPWS_CURR}
            if ((GetAccurateTime - mTmpTime) < 300.0) then Exit ; // 300ms 주기
            {$ELSE}
            if ((GetAccurateTime - mTmpTime) < 1000.0) then Exit ;
            {$ENDIF}
            ...
            // [수정] 상태 순환(mod) 값 변경
            {$IFDEF USEPWS_CURR}
            msubProc:= msubProc mod 4 ; // _ORD_POWER, _ORD_VOLT, _ORD_CURR, _ORD_MEAS_CURR
            {$ELSE}
            msubProc:= msubProc mod 3 ;
            {$ENDIF}

            if not GetComStatus then msubProc:= 0 ; //통신응답이 없을 경우 진행안함

            if msubProc = 0 then mProc:= 1
            else                 mProc:= 2 ;
        end ;
        5:
        begin
            ...
            begin
                ...
                begin
                    ...
                    SetErrorTxt(Format('%s Power Supply', [GetDevComName(mDevCom.DevID)]),
                                _TR('설정을 할 없습니다.!'),
                                _TR('파워서플라이 전원을 리셋후 검사 프로그램을 다시 시작해주십시요'),
                                umtError);
                end ;
                mProc:= _ER_PROC ;
                Exit ;
        ...
        8:
        begin
            if (GetaccurateTime - mTmpTime) < 5000.0 then Exit ;
            //gLog.Panel('PROC = %d, %s',[mProc, GetDevComName(mDevCom.DevID)]);
            mProc:= 1 ;
        end ;
        9:;
...
{$IFDEF VIRTUALIO}
    Result := mVolt;
{$ELSE}
    if mOutPower then Result:= mVolt
    else              Result:= 0.0 ;
{$ENDIF}
end;
...
begin
    for i := 0 to Length(mItems) - 1 do
        if mItems[i].IsRun then Exit(true);
