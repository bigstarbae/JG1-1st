...
type
    TfrmPwrBurnishing = class(TfrmBaseMain)
        ...
        lblSlideLimitStatus: TLabel3D;
        pnlBurnHgt: TPanel;
        lblHgtBrnCnt: TLabel3D;
        lbl1: TLabel3D;
        lblHgtLimitStatus: TLabel3D;
        pnlBurnTILT: TPanel;
        lblTILTBrnCnt: TLabel3D;
        lbl2: TLabel3D;
        lblTILTLimitStatus: TLabel3D;
        pnlBurnLegSupt: TPanel;
        lblLegSuptBrnCnt: TLabel3D;
        lbl3: TLabel3D;
        lblLegSuptLimitStatus: TLabel3D;
        pnlBurnSwivel: TPanel;
        lblSwivelBrnCnt: TLabel3D;
        lbl4: TLabel3D;
        lblSwivelLimitStatus: TLabel3D;
        pnlTestTitle: TPanel;
        lbl: TLabel3D;
        lblBrnCount: TLabel3D;
        lblLimitStatus: TLabel3D;
    ...
    { Private declarations }
        ...
        procedure SetLimitStatus(Motor: TSeatMotor);
...
uses
    ...
    GraphconfigForm, BaseDIO, BaseAD, ErrorForm;
...
procedure TfrmPwrBurnishing.ChartAfterDraw(Sender: TObject);
begin
     // 안전 체크 추가
...
procedure TfrmPwrBurnishing.DrawHorizontalLine(YValue: Double; LineColor: TColor; LineWidth: Integer);
var
    yPos: Integer;
begin
...
{$ENDIF}
    ...
    for i := 0 to mTsWork.ConCount - 1 do
        tcCon.Tabs.Add(IfThen(mTsWork.Connectors[i].IsSwType, 'SWITCH', 'MAIN'));
...
procedure TfrmPwrBurnishing.OnWorkStatus(AStatus: Integer);
    ...
    begin
        ...
        begin
            ...
            SetLimitStatus(mTsWork.Motors[MtrIt]);
        end;

        pnlBurnSlide.Visible := mTswork.CurModelType.IsSlideExists;
        pnlBurnTILT.Visible := mTswork.CurModelType.IsTiltExists;
        pnlBurnHgt.Visible := mTswork.CurModelType.IsHeightExists;
        pnlBurnLegSupt.Visible := mTswork.CurModelType.IsLegSuptExists;
        pnlBurnSwivel.Visible := mTswork.CurModelType.isSwivelExists;

        lblBrnCount.Visible := not mTsWork.CurModelType.IsIMS;
        lblSlideBrnCnt.Visible := not mTsWork.CurModelType.IsIMS;
        lblTILTBrnCnt.Visible := not mTsWork.CurModelType.IsIMS;
        lblHgtBrnCnt.Visible := not mTsWork.CurModelType.IsIMS;
        lblLegSuptBrnCnt.Visible := not mTsWork.CurModelType.IsIMS;
        lblSwivelBrnCnt.Visible := not mTsWork.CurModelType.IsIMS;

        lblLimitStatus.Visible := mTsWork.CurModelType.IsIMS;
        lblSlideLimitStatus.Visible := mTsWork.CurModelType.IsIMS;
        lblTILTLimitStatus.Visible := mTsWork.CurModelType.IsIMS;
        lblHgtLimitStatus.Visible := mTsWork.CurModelType.IsIMS;
        lblLegSuptLimitStatus.Visible := mTsWork.CurModelType.IsIMS;
        lblSwivelLimitStatus.Visible := mTsWork.CurModelType.IsIMS;
        ...
        case AStatus of
            ...
            ord(tw_START):
                begin
                    ...
                    lblStatus.Caption := _TR('검사 시작, 리미트 삭제 ');
                    lblResult.Caption := '▶';
                    mCT.Start;
                end;
            ord(tw_STOP):
            ...
            ord(tw_LIMIT_START):
                begin
                    lblStatus.Caption := _TR('리미트 설정 시작');

                end;

            ord(tw_LIMIT_DONE):
                begin
                    lblStatus.Caption := _TR('리미트 설정 완료');
                    for MtrIt := Low(TMotorORD) to MtrOrdHi do
                        if mTsWork.Motors[MtrIt].Use then
                            SetLimitStatus(mTsWork.Motors[MtrIt]);
                    HideMsg;
...
procedure TfrmPwrBurnishing.SetLimitStatus(Motor: TSeatMotor);
begin
    case Motor.ID of
        tmSlide:
            begin
                lblSlideLimitStatus.Caption := Motor.IMSCtrler.GetLimitStatusStr(Motor.ID);
            end;
        tmTilt:
            begin
                lblTILTLimitStatus.Caption := Motor.IMSCtrler.GetLimitStatusStr(Motor.ID);
            end;
        tmHeight:
            begin
                lblHgtLimitStatus.Caption := Motor.IMSCtrler.GetLimitStatusStr(Motor.ID);
            end;
        tmLegSupt:
            begin
                lblLegSuptLimitStatus.Caption := Motor.IMSCtrler.GetLimitStatusStr(Motor.ID);
            end;
        tmSwivel:
            begin
                lblSwivelLimitStatus.Caption := Motor.IMSCtrler.GetLimitStatusStr(Motor.ID);
            end;
    end;
end;
...
begin
    case Motor.ID of
        ...
        tmTilt:
            begin
                lblTILTBrnCnt.Caption := Format('%d/%d', [Motor.CurRepeatCount, Motor.TotRepeatCount]);
            end;
        tmHeight:
            begin
                lblHgtBrnCnt.Caption := Format('%d/%d', [Motor.CurRepeatCount, Motor.TotRepeatCount]);
            end;
        tmLegSupt:
            begin
                lblLegSuptBrnCnt.Caption := Format('%d/%d', [Motor.CurRepeatCount, Motor.TotRepeatCount]);
            end;
        tmSwivel:
            begin
                lblSwivelBrnCnt.Caption := Format('%d/%d', [Motor.CurRepeatCount, Motor.TotRepeatCount]);
            end;
    end;
end;
