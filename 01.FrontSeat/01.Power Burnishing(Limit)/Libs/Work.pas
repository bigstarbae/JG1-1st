...
const
    ...
    ARY_HI_DAQ_VALS: array[0..8 - 1] of double = (100, 100, 10, 10, 10, 10, 10, 10);
    ARY_LO_DAQ_VALS: array[0..8 - 1] of double = (-100, -100, -10, -10, -10, -10, -10, -10);
...
begin
    ...
    begin
        ...
        ADs[i] := TUserECAD.Create([TECModule.Create(OffsetIn + 16, 8),
                                    TECModule.Create(OffsetIn + 36, 8),
        ...
        DIOs[i] := TEtherCatDIO.Create([TECModuleDIO.Create(OffsetIn + 4, 12)],
...
{$ELSE}
        CANs[i] := CreateCAN(cmtPeak, i);   // PeakCAN 사용
{$ENDIF}
        ...
        gTsPwrWorks[i] := TTsWork.Create(StID, ADs[i], DIOs[i], CANs[i], AOs[i], nil);
...
begin
    ...
    begin
        TECManager.GetInstance.Delay := false;
        TECManager.GetInstance.Start;

        TDAQManager.GetInstance.Delay := false;
