...
const
    ST_EC_IN_BYTE_COUNT = 204;
    ST_EC_OUT_BYTE_COUNT = 20;
    ARY_HI_DAQ_VALS: array[0..7] of double = (100, 100, 60, 60, 100, 1000, 100, 100);
    ARY_LO_DAQ_VALS: array[0..7] of double = (-100, -100, -60, -60, -100, -1000, -100, -100);
...
begin
    ...
    gLanDIO := TDioMonitor.Create(Handle, [TPlcAddInfo.Create(dtWORD, 'W', PlcIOAdd.mRead, _WORD_PER_ST * MAX_ST_COUNT, 0), TPlcAddInfo.Create(dtWORD, 'W', PlcIOAdd.mData, MAX_MODEL_WD_COUNT, 1, True)], [TPlcAddInfo.Create(dtWORD, 'W', PlcIOAdd.mWrite, _WORD_PER_ST * MAX_ST_COUNT, 0)]);
...
begin
    ...
    begin
        ...
        ADs[i] := TUserECAD.Create([TECModule.Create(OffsetIn + 26, 8), TECModule.Create(OffsetIn + 46, 8), TECModule.Create(OffsetIn + 66, 8), TECModule.Create(OffsetIn + 86, 8), TECModule.Create(OffsetIn + 106, 8), TECModule.Create(OffsetIn + 126, 8), TECModule.Create(OffsetIn + 146, 8), TECModule.Create(OffsetIn + 166, 8), TECModule.Create(OffsetIn + 186, 8)]);

        ADs[i].Init(ARY_DAQ_GAINS, ARY_HI_DAQ_VALS, ARY_LO_DAQ_VALS, 0, 7);
        ADs[i].Init(1, 10 * 2, -10 * 2, Abs(SYS_REF_VOLT), AI_MAIN_CON_VOLT_START, AI_HEADMTR_CON_VOLT_END);    ///
        ...
        DIOs[i] := TEtherCatDIO.Create([TECModuleDIO.Create(OffsetIn + 4, 12)], [TECModuleDIO.Create(OffsetOut + 8, 12), TECModuleDIO.Create(OffsetOut + 10, 12), TECModuleDIO.Create(OffsetOut + 12, 12), TECModuleDIO.Create(OffsetOut + 14, 12), TECModuleDIO.Create(OffsetOut + 16, 12), TECModuleDIO.Create(OffsetOut + 18, 12)], _DIO_OUT_START);
