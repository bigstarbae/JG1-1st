...
uses
    ...
    TeeProcs, Chart, SeatMtrTestFrame, HVDataFrame;

type
    TfrmSoundProof = class(TfrmBaseMain)
        pnlClient: TPanel;
        shpTop: TShape;
        shpLeft: TShape;
        shpRight: TShape;
        ...
        tcCon: TTabControl;
        pnlMtr: TPanel;
        fmMtr1: TSeatMtrTestFrme;
        SeatMtrTestFrme1: TSeatMtrTestFrme;
        SeatMtrTestFrme2: TSeatMtrTestFrme;
        SeatMtrTestFrme3: TSeatMtrTestFrme;
        pnlElec: TPanel;
        pnlHV: TPanel;
        pnlHeat: TPanel;
        pnlVent: TPanel;
        ...
        Label3D3: TLabel3D;
        Panel10: TPanel;
        lblPSUVerHw: TLabel3D;
        lblPSUVerSw: TLabel3D;
        lblPSUPartNo: TLabel3D;
        Label3D4: TLabel3D;
        Panel13: TPanel;
        lblSHVUVerSw: TLabel3D;
        lblSHVUPartNo: TLabel3D;
        Label3D5: TLabel3D;
        Label3D16: TLabel3D;
        lblSHVUVerHw: TLabel3D;
        Panel20: TPanel;
        Panel23: TPanel;
        Label9: TLabel;
        Panel24: TPanel;
        ...
        Panel27: TPanel;
        Label3D34: TLabel3D;
        HVDatFrame1: THVDatFrame;
        HVDatFrame3: THVDatFrame;
        Label3D1: TLabel3D;
        Label3D2: TLabel3D;
        Panel2: TPanel;
        Panel7: TPanel;
...
procedure TfrmSoundProof.chtMtrDblClick(Sender: TObject);
begin
...
procedure TfrmSoundProof.DisplayDiskFree;
var
    dTm: double;
    iTm, Total: int64;
begin
...
procedure TfrmSoundProof.FormClose(Sender: TObject; var Action: TCloseAction);
begin
    inherited;
    FreeAndNil(abPLC);
    FreeAndNil(gWork);
...
procedure TfrmSoundProof.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
    inherited;
...
procedure TfrmSoundProof.FormCreate(Sender: TObject);
begin
    inherited;
...
procedure TfrmSoundProof.FormDestroy(Sender: TObject);
begin
...
procedure TfrmSoundProof.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
var
    i: Integer;
...
procedure TfrmSoundProof.FormShow(Sender: TObject);
var
    i: Integer;
...
{$ENDIF}
    ...
    begin
        ...
        AddSubForm(TfrmSoundProof.Create(Self)).ShowPLCInfo(False);
    end;
...
procedure TfrmSoundProof.GlobalExceptionHandler(Sender: TObject; E: Exception);
begin
...
procedure TfrmSoundProof.HandleReadMessage(var myMsg: TMessage);
begin
...
procedure TfrmSoundProof.lblSimLanInModeClick(Sender: TObject);
begin
...
procedure TfrmSoundProof.OnWorkStatus(AStatus: Integer);
...
procedure TfrmSoundProof.ShowConnectorForm(Station: integer);
begin
...
procedure TfrmSoundProof.SpeedButton1Click(Sender: TObject);
begin
    inherited;
    ShowMessage('TEST');
end;

procedure TfrmSoundProof.sbtnConClick(Sender: TObject);
begin
    ShowConnectorForm(Ord(mTsWork.StationID));
end;

procedure TfrmSoundProof.sbtnExitClick(Sender: TObject);
begin
...
procedure TfrmSoundProof.sbtnModelClick(Sender: TObject);
begin
...
procedure TfrmSoundProof.sbtnReferenceClick(Sender: TObject);
begin
    if not ShowPasswordForm then
        Exit;
    ...
    frmUserRefer := TfrmUserRefer.Create(Self, [], [riMotor, riSoundProof, riBackAngle, riElec, riBuckle, riIMSLimit, riIMSMem]);
    frmUserRefer.FormStyle := fsStayOnTop;
    frmUserRefer.Show();

end;

procedure TfrmSoundProof.sbtnRetrieveClick(Sender: TObject);
var
    sTm: string;
    nP: IProcesses;
    nF: TfrmWait;
...
procedure TfrmSoundProof.SetBurnishingCount(Motor: TSeatMotor);
begin
    case Motor.ID of
        tmSlide:
            begin
...
procedure TfrmSoundProof.sgConDrawCell(Sender: TObject; ACol, ARow: Integer; Rect: TRect; State: TGridDrawState);
var
...
procedure TfrmSoundProof.tcConChange(Sender: TObject);
begin
    if tcCon.TabIndex = 0 then
        UpdateConUI(mTsWork.MainCon);
end;

procedure TfrmSoundProof.tmrDiskChkTimer(Sender: TObject);
begin
    inherited;
    DisplayDiskFree;
end;

procedure TfrmSoundProof.tmrMainTimer(Sender: TObject);
var
    dTm: double;
    sTm: string;
begin
...
procedure TfrmSoundProof.tmrPollTimer(Sender: TObject);
begin
...
procedure TfrmSoundProof.tmrShowTimer(Sender: TObject);
begin
    inherited;
...
procedure TfrmSoundProof.UpdateConUI(Con: TBaseSeatConnector);
var
...
procedure TfrmSoundProof.WorkClose(Sender: TObject);
begin
    gWork.Thread := nil;
    Close;
end;

procedure TfrmSoundProof.WorkError(Sender: TObject);
begin
    gWork.Thread := nil;
...
procedure TfrmSoundProof.ReadCount;
var
    nOK, nNG: Integer;
begin
...
procedure TfrmSoundProof.ReadDAQValues;
var
...
procedure TfrmSoundProof.ReadWorkCycleTime;
const
