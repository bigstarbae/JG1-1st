...
type
    TfrmLimit = class(TfrmBaseMain)
        ...
        shpGap2: TShape;
        Shape3: TShape;
        pnlMtrCurrLegend: TPanel;
        lblMtrCurrCaption: TLabel;
        Shape2: TShape;
        chtMtr: TChart;
        Series1: TFastLineSeries;
        pnlLimitInfo: TPanel;
        pnlLimit_Slide: TPanel;
        Label3D2: TLabel3D;
        lblLimitStatus_Slide: TLabel3D;
        pnlLimit_CushTilt: TPanel;
        lbl1: TLabel3D;
        lblLimitStatus_CushTilt: TLabel3D;
        pnlLimit_Recl: TPanel;
        el12: TLabel3D;
        lblLimitStatus_Recl: TLabel3D;
        pnlLimit_Shoulder: TPanel;
        lbl3: TLabel3D;
        lblLimitStatus_Shoulder: TLabel3D;
        pnlLimit_Relax: TPanel;
        lbl4: TLabel3D;
        lblLimitStatus_Relax: TLabel3D;
        shpLegend_Slide: TShape;
        shpLegend_CushTilt: TShape;
        shpLegend_Recl: TShape;
        shpLegend_Shoulder: TShape;
        shpLegend_Relax: TShape;
        pnlLimit_WalkinTilt: TPanel;
        Label3D1: TLabel3D;
        Label3D3: TLabel3D;
        shpLegend_WalkinTilt: TShape;
        Label3D4: TLabel3D;
        ...
        procedure pnlSysStatusClick(Sender: TObject);
    ...
    { Private declarations }
        ...
        procedure DrawHorizontalLine(YValue: Double; LineColor: TColor; LineWidth: Integer);
...
procedure TfrmLimit.ChartAfterDraw(Sender: TObject);
begin
     // 안전 체크 추가
...
//    DrawHorizontalLine(mTsWork.Motors[tmSlide].LockCurr, $00AAAAFF, 1);
//
//    if mTsWork.Motors[tmSlide].LockCurr <> mTsWork.Motors[tmHeight].LockCurr then
//        DrawHorizontalLine(mTsWork.Motors[tmHeight].LockCurr, $00AAAAFF, 1);
end;

procedure TfrmLimit.chtMtrDblClick(Sender: TObject);
begin
...
procedure TfrmLimit.DisplayDiskFree;
var
    dTm: double;
    iTm, Total: int64;
begin
...
procedure TfrmLimit.DrawHorizontalLine(YValue: Double; LineColor: TColor;
  LineWidth: Integer);
var
    yPos: Integer;
begin
    with chtMtr.Canvas do
    begin
        Pen.Color := LineColor;
        Pen.Width := LineWidth;
        yPos := chtMtr.Axes.Left.CalcYPosValue(YValue);
        MoveTo(chtMtr.Axes.Bottom.CalcXPosValue(chtMtr.Axes.Bottom.Minimum), yPos);
        LineTo(chtMtr.Axes.Bottom.CalcXPosValue(chtMtr.Axes.Bottom.Maximum), yPos);
    end;
...
procedure TfrmLimit.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
    inherited;
...
procedure TfrmLimit.FormCreate(Sender: TObject);
begin
    inherited;
...
procedure TfrmLimit.FormDestroy(Sender: TObject);
begin
    mSC.Free;
...
procedure TfrmLimit.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
var
    i: Integer;
...
{$ENDIF}
    ...
    begin
        ...
        AddSubForm(TfrmLimit.Create(Self)).ShowPLCInfo(False);
    end;
...
procedure TfrmLimit.GlobalExceptionHandler(Sender: TObject; E: Exception);
begin
...
procedure TfrmLimit.HandleReadMessage(var myMsg: TMessage);
begin
...
procedure TfrmLimit.lblSimLanInModeClick(Sender: TObject);
begin
...
procedure TfrmLimit.OnWorkStatus(AStatus: Integer);
...
//        end;
        ...
        pnlLimit_Slide.Visible := AModel.rTypes.IsSlideMtrExists;
        pnlLimit_CushTilt.Visible := AModel.rTypes.IsCushTiltMtrExists;
        pnlLimit_WalkinTilt.Visible := AModel.rTypes.IsWalkinTiltMtrExists;
        pnlLimit_Recl.Visible := AModel.rTypes.IsReclineMtrExists;
        pnlLimit_Relax.Visible := AModel.rTypes.IsRelaxMtrExists;
        pnlLimit_Shoulder.Visible := AModel.rTypes.IsShoulderMtrExists;

        RefreshAlignedControls([pnlLimit_Slide, pnlLimit_CushTilt, pnlLimit_WalkinTilt, pnlLimit_Recl, pnlLimit_Relax, pnlLimit_Shoulder], alTop);
...
procedure TfrmLimit.pnlSysStatusClick(Sender: TObject);
var
    Vis: Boolean;
begin
    inherited;
    shpLegend_Slide.Visible := not Vis;
    shpLegend_CushTilt.Visible := not Vis;
    shpLegend_WalkinTilt.Visible := not Vis;
    shpLegend_Recl.Visible := not Vis;
    shpLegend_Shoulder.Visible := not Vis;
    shpLegend_Relax.Visible := not Vis
end;
...
procedure TfrmLimit.ShowConnectorForm(Station: integer);
begin
...
procedure TfrmLimit.SpeedButton1Click(Sender: TObject);
begin
    inherited;
    ShowMessage('TEST');
end;

procedure TfrmLimit.sbtnConClick(Sender: TObject);
begin
    ShowConnectorForm(Ord(mTsWork.StationID));
end;

procedure TfrmLimit.sbtnExitClick(Sender: TObject);
begin
...
procedure TfrmLimit.sbtnModelClick(Sender: TObject);
begin
...
procedure TfrmLimit.sbtnReferenceClick(Sender: TObject);
begin
    if not ShowPasswordForm then
        Exit;
    ...
    frmUserRefer := TfrmUserRefer.Create(Self, [uriPwrSpl, uriECU], [riMotor, riIMSLimit]);
    frmUserRefer.FormStyle := fsStayOnTop;
    frmUserRefer.Show();

end;

procedure TfrmLimit.sbtnRetrieveClick(Sender: TObject);
var
    sTm: string;
    nP: IProcesses;
    nF: TfrmWait;
...
procedure TfrmLimit.sgConDrawCell(Sender: TObject; ACol, ARow: Integer; Rect: TRect; State: TGridDrawState);
var
...
procedure TfrmLimit.tcConChange(Sender: TObject);
begin
    if tcCon.TabIndex = 0 then
        UpdateConUI(mTsWork.MainCon);
end;

procedure TfrmLimit.tmrDiskChkTimer(Sender: TObject);
begin
    inherited;
    DisplayDiskFree;
end;

procedure TfrmLimit.tmrMainTimer(Sender: TObject);
var
    dTm: double;
    sTm: string;
begin
...
procedure TfrmLimit.tmrPollTimer(Sender: TObject);
begin
    mSC.Add([mTsWork.GetMotorCurr, 0, 0, 0]);
...
procedure TfrmLimit.tmrShowTimer(Sender: TObject);
begin
    inherited;
...
procedure TfrmLimit.UpdateConUI(Con: TBaseSeatConnector);
var
...
procedure TfrmLimit.UpdateGraph(IsClear: Boolean);
...
procedure TfrmLimit.WorkClose(Sender: TObject);
begin
    gWork.Thread := nil;
    Close;
end;

procedure TfrmLimit.WorkError(Sender: TObject);
begin
    gWork.Thread := nil;
...
procedure TfrmLimit.ReadDAQValues;
var
...
procedure TfrmLimit.ReadWorkCycleTime;
const
