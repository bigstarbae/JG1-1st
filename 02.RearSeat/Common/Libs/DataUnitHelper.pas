...
interface
uses
    DataUnitOrd, SysUtils, Generics.Collections, HVTester, SeatMotorType;
    ...
    function GetDataRO(MtrID: TMotorOrd; ItemName: string; Dir: TMotorDir): TResultOrd;
    function GetDevRO(MtrID: TMotorOrd; ItemName: string; Dir: TMotorDir): TResultOrd; // 소음편차(Deviation) 전용

    function GetRsRO(MtrID: TMotorOrd; ItemName: string; Dir: TMotorDir): TResultOrd; overload;
    function GetRsRO(MtrID: TMotorOrd; ItemName: string): TResultOrd; overload;
...
var
    Str: string;
begin
    Str := 'roData' + ItemName + _MOTOR_DIR_STR[Dir] + '_' + _MOTOR_ID_STR[MtrID];
    Result := GetResultOrdByName(Str);
end;

// 소음 편차 전용
function GetDevRO(MtrID: TMotorOrd; ItemName: string; Dir: TMotorDir): TResultOrd;
var
    Str: string;
begin
    Str := 'roDev' + ItemName + _MOTOR_DIR_STR[Dir] + '_' + _MOTOR_ID_STR[MtrID];
    Result := GetResultOrdByName(Str);
end;

function GetRsRO(MtrID: TMotorOrd; ItemName: string; Dir: TMotorDir): TResultOrd;
var
    Str: string;
begin
    Str := 'roRs' + ItemName + _MOTOR_DIR_STR[Dir] + '_' + _MOTOR_ID_STR[MtrID];
    Result := GetResultOrdByName(Str);

end;

function GetRsRO(MtrID: TMotorOrd; ItemName: string): TResultOrd;
var
    Str: string;
begin
    Str := 'roRs' + ItemName  + '_' + _MOTOR_ID_STR[MtrID];
    Result := GetResultOrdByName(Str);
...
begin
    if IsOn then
        Result := GetResultOrdByName('roRs' + THVDevNameStr[HVDevType] + 'On' + THVDevPosStr[HVDevType])
    else
        Result := GetResultOrdByName('roRs' + THVDevNameStr[HVDevType] + 'Off' + THVDevPosStr[HVDevType]);
