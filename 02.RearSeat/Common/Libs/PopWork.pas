...
uses
    ...
    LanIoUnit, DataUnit, DataUnitOrd, Generics.Collections, ModelType, SeatType, BlckSock,
    TimeChecker;
{
사양정보
구분자	"@"
...
type
    ...
    //RcvDatas -----------------
        ...
        mrcvLotNo, mrcvLclPartNo,              // 자체 품번
        mrcvPalletNo,                //PalletNo: xxxx~~
        mrcvWorkType,                //0-NONE, 1-Exists, 2: Rework ,3: 통과
        mrcvWorkDetail, mrcvDrive, mrcvPosType, mrcvSeatType, mrcvImsType, mrcvOptType, mrcvOptType2,  // 운전, 조수석 히터/밴트
        mrcvPodsType, mrcvCarType, mrcvLumbType, mrcvEcuType, mrcvLocal,                   //지역
        mrcvColor,                   //색상
        mrcvMotor,                   // 모터 사용 유무 : 0000
        mrcWorkIn, mrcvElecResult              // 전장 검사 결과  :
            : string;
        ...
        mLatencyTime, mPoprcvTime: double;
    //------------------------
    //2007.06.11
        mUsePop: boolean;
    //2008.06.16
...
uses
    Global, Log, myUtils, DataUnitHelper, LangTran, SysEnv;
    ...
    begin
        try
            ...
            begin
                ...
                begin
                    if (rCommand in [SYS_CMD_WRITE_DATA]) and Assigned(rOnReceive) then
                    begin
...
function TPopSystem.AddData(const AOnReceive: TRcvPopData; ACommand: integer; const AData: string): boolean;
var
    pBuf: pPopData;
begin
    Result := false;
    ...
    begin
        ...
        rCommand := ACommand;
        if AData <> '' then
            StrLCopy(rWriteData, PChar(AData), 511);
    end;
...
begin
    ...
    mClient.IP := gSysEnv.rComm.rPop.rHostIp;
    mClient.Port := gSysEnv.rComm.rPop.rPort + mTag;
    mConnectTime := gSysEnv.rComm.rPop.rConnectTime;
    mSendTerm := gSysEnv.rComm.rPop.rSendTerm;
    mReConOfSndFault := gSysEnv.rComm.rPop.rReConnect;
    mUsePop := gSysEnv.rComm.rPop.rUse;
    mResponseTime := 3;
    mNotsndResult := gSysEnv.rComm.rPop.rSndData;
end;
...
begin
    if mProc in [_CONNECT_WAIT] then
        mProc := _CHECK_DATA;
    ...
    if Assigned(mRcvNotify) then
        OnRcvData(Self, SYS_POP_CONNECTED);
end;
...
begin
    ...
    if Assigned(mRcvNotify) then
        OnRcvData(Self, SYS_POP_CONNECTING);
end;
...
begin
    ...
    ClearList;
    if Assigned(mRcvNotify) then
        OnRcvData(Self, SYS_POP_DISCONNECTED);
...
begin
    ...
    if AData <> _SEND_CHECK then
        gLog.Panel('IP:%s:%d POP%d send: %s', [mClient.IP, mClient.Port, mTag, AData]);
...
begin
    ...
    case pBuf^.rCommand of
        SYS_CMD_WRITE_DATA:
            begin
                Result := Format('%s%s%d%s%s%s%s', [_HEAD, _TOKEN, pBuf^.rCommand, _TOKEN, pBuf^.rWriteData, _TOKEN, _TAIL]);
                IsReturn := true;
            end;
        SYS_REQ_MODEL:
            begin
    ...
    begin
        ...
        try
            ...
            begin
                with pBuf^ do
                begin
                    if (rCommand = SYS_CMD_WRITE_DATA) and Assigned(rOnReceive) then
                    begin
                        rOnReceive(Self, SYS_CMD_RCV_ERROR);
                    end;
                end;
...
begin
    ...
    if mReset and (mProc in [_CONNECTING.._DELAY_TIME]) then
    begin
        mReset := false;
        mProc := _LOAD_ENV;
    end;
...
begin
    ...
    if (iH > iT) then
        Delete(AData, 1, iH - 1)
    else if (iH > 0) and (iT > iH) then
    begin
        AData := Copy(AData, iH + 1, iT - iH - 1);
        Result := true;

    end
    else if (iT <= 0) and (iH <= 0) then
        AData := '';
            ...
            _TOKEN:
                begin
                    case iT of
                        ...
                        2:
                            mrcvsysCode := Trim(sTm);
                        3:
                            mrcvPartNo := Trim(sTm);
                        4:
                            mrcvLotNo := Trim(sTm);
                        5:
                            mrcvWorkType := Trim(sTm);
                        6:
                            mrcvWorkDetail := Trim(sTm);
                    ...
                    // 아래 사용 안 함...
                        ...
                        7:
                            mrcvDrive := Trim(sTm);
                        8:
                            mrcvPosType := Trim(sTm);
                        9:
                            mrcvSeatType := Trim(sTm);
                        10:
                            mrcvImsType := TRim(sTm);
                        11:
                            begin
                                mrcvOptType := Trim(sTm);
                            end;
                        12:
                            mrcvPodsType := Trim(sTm);
                        13:
                            mrcvCarType := Trim(sTm);
                        14: {mrcvLumbType := Trim(sTm)}
                            ;
                        15:
                            mrcvEcuType := Trim(sTm);
                        16:
                            mrcvLocal := Trim(sTm);
                        17:
                            mrcvColor := Trim(sTm);
...
begin
    ...
    if (Count <= 0) or (ItemCount > Count) then
        Exit(0);
    ...
    begin
        TsOrd := ResultOrd2TsOrd(PopDataOrd[i + _MNR_COUNT]);
            ...
            case mrcvCommand of
                SYS_CMD_RCV_MODEL:
                    begin
                        ...
                        bTm := true;
                        if Assigned(mRcvNotify) then
                            OnRcvData(Self, mrcvCommand);
                        if bTm then
                        begin
                            gLog.Panel('POP수신(모델) 성공(%s)', [mrcvCarType]);
                        end;
                        SendData(Format('%s%s%d%s%s', [_HEAD, _TOKEN, SYS_CMD_RCV_ACK, _TOKEN, _TAIL]));

                    //모델요청에 대한 응답.
                        if mList.Count > 0 then
                        begin
                            pBuf := mList.Items[0];
                            if Assigned(pBuf) and (pBuf^.rCommand = SYS_REQ_MODEL) then
                            begin
                                mRepeat := 0;
                ...
                SYS_CMD_RCV_ACK:
                    begin
                        ...
                        begin
                            pBuf := mList.Items[0];
                            if Assigned(pBuf) and (pBuf^.rCommand = SYS_CMD_WRITE_DATA) then
                            begin
                                mrcvCommand := SYS_CMD_RCV_ACK;
                ...
                SYS_CMD_PING:
                    begin
                        SendData(Format('%s%s%d%s%s', [_HEAD, _TOKEN, SYS_CMD_RCV_EX_ACK, _TOKEN, _TAIL]));
...
begin
    ...
    if not (iDx in [ord(Low(TCAR_TYPE))..Ord(High(TCAR_TYPE))]) then
        Exit;
...
begin
    ...
    if not (iDx in [ord(Low(TPOS_TYPE))..ord(High(TPOS_TYPE))]) then
        Exit;
