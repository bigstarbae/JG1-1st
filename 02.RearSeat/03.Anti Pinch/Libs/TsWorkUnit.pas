...
type
    ...
    protected
        ...
        procedure PowerOnOff(IsOn: Boolean);
...
begin
    ...
    if mDIO.IsIO(DO_TEST_POWER) then
        Result := True;
...
begin
    ...
    gLog.Panel('모터 사용: S:%s, C.T:%s, W.T:%s, R:%s, Rx:%s, Sh:%s',
        [BoolToStr(mMotors[tmSlide].Use, True), BoolToStr(mMotors[tmCushTilt].Use, True),
        BoolToStr(mMotors[tmWalkinUpTilt].Use, True), BoolToStr(mMotors[tmRecl].Use, True),
        BoolToStr(mMotors[tmRelax].Use, True), BoolToStr(mMotors[tmShoulder].Use, True)]);
    ...
    case mSpecChkState of
        ...
        1:
            begin
                ...
                mDIO.SetIO(DO_SPEC_CHK_POWER, True);
...
{$ENDIF}
        ...
        5:
            begin
                ...
                mDIO.SetIO(DO_SPEC_CHK_POWER, False);
                PowerOnOff(True);
                Inc(mSpecChkState);
            end;
        6:
...
{$ENDIF}
                mMtrOper.Init(
                    [mMtrSlide, mMtrRecline, mMtrCushTilt, mMtrWalkinUpTilt, mMtrShoulder, mMtrRelax],
                    [twBack, twBack, twForw, twForw, twForw, twForw]);
                mMtrOper.ClearFSM;
                mFuncState := 0;
...
procedure TTsWork.PowerOnOff(IsOn: Boolean);
begin
    mDIO.SetIO(DO_TEST_POWER, IsOn);
    if not IsOn then
        mDIO.SetIO(DO_SPEC_CHK_POWER, False);
end;
